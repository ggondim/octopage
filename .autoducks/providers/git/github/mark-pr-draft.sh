...
# git::mark_pr_draft PR_NUMBER [SLUG] [TOKEN]
...
git::mark_pr_draft() {
  ...
  GH_TOKEN="$token" gh pr ready "$pr_number" --repo "$slug" --undo
}

if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  case "${1:-}" in
    --help) echo "Usage: git::mark_pr_draft PR_NUMBER [SLUG] [TOKEN]"; echo "  Convert a ready pull request back to draft"; echo "  SLUG defaults to \$REPO, TOKEN defaults to the ambient GH_TOKEN"; echo "  Requires: REPO env var (when SLUG omitted)"; exit 0 ;;
  esac
fi
