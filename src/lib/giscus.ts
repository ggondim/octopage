...
import type { DiscussionCategory } from './github/discussions.ts';

/**
 * Pick the category that holds comment threads.
 *
 * There is no API to create a discussion category — not in GraphQL, not in
 * REST — so this can only ever choose among categories that already exist, and
 * the failure has to be an actionable message rather than a silent fallback.
 *
 * `Announcements` is preferred because giscus recommends it: only maintainers
 * can open threads there, so readers cannot create arbitrary discussions
 * through the widget.
 */
export function resolveCommentsCategory(
  categories: DiscussionCategory[],
  preferred?: string,
): DiscussionCategory {
  if (preferred) {
    const match = categories.find((c) => c.name.toLowerCase() === preferred.toLowerCase());
    if (match) return match;
    throw new Error(
      `octopage.config.ts sets comments: '${preferred}', but this repository has no such discussion category.\n` +
        `Available: ${categories.map((c) => c.name).join(', ') || '(none)'}\n` +
        `Categories cannot be created through the API — add it under Settings → Discussions.`,
    );
  }

  const announcements = categories.find((c) => c.name.toLowerCase() === 'announcements');
  if (announcements) return announcements;

  const usable = categories.find((c) => !c.isAnswerable);
  if (usable) return usable;

  throw new Error(
    'No usable discussion category for comments.\n' +
      'Create one under Settings → Discussions (an announcement-style category is recommended, ' +
      'so only maintainers can open threads), or set `comments` in octopage.config.ts.',
  );
}
